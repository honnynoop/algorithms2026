...
public class Main {
	...
	private static void go(int[] numss) {
		...
		for (int i = 0; i < M; i++) {
			...
			copy[cur[0]][cur[1]]=0;  // 바이러스 위치에서 시작할때
		}
		while(!que.isEmpty()){
			int[] cu=que.poll();
			int cr=cu[0];
			...
			for (int d = 0; d < 4; d++) {
				...
				if(copy[nr][nc]==-2){  // 온적 없다. empty -2
					...
					if(copy[cr][cc]<=-4){ 
						copy[nr][nc]=-copy[cr][cc]-3+1;//6-3+1
					}else if(copy[cr][cc]>=0){
						copy[nr][nc]=copy[cr][cc]+1;
					}
				}else if(copy[nr][nc]==-3){  // 비활성화 -3* -> -4 활성화
					que.offer(new int[]{nr,nc});
					if(copy[cr][cc]>0){ 
						copy[nr][nc]=-(copy[cr][cc]+1+3);//  -3 1
					}else if(copy[cr][cc]<=-4){ 
						copy[nr][nc]=copy[cr][cc]-1;
					}else if(copy[cr][cc]==0){// 시작이거나 
						copy[nr][nc]=-4;
					}
				}
			}
